---
id: src_01KGAGQ9N3VXZFMSXFMDSZWNGF
title: jj_gateway_v_0
ingested_at: 2026-01-31T17:13:31.043861+00:00
...
2) **CLI client**: `jj chat` provides a REPL with streaming assistant responses.
3) **Telegram bot**: `jj gateway` can also run a Telegram adapter using long polling.
4) **Persistence**:
...
- **Gateway**: daemon process `jj gateway`.
...
2) **CLI Adapter**
- `jj chat` connects to Gateway, opens a session, sends user messages, displays streaming events.
...
- provide `jj sessions` and `jj tail <session_key>` later
...
**Acceptance:** `jj chat` shows streaming; transcript has final assistant.
...
### 17.2 Gateway daemon `jj gateway` (required)
- Binary: `jj` with subcommand `gateway`.
- Runs a local control-plane server (WS) on loopback.
- Loads config from `~/.jj/gateway/config.toml` (overridable via `--config`).
- Creates/uses data dir `~/.jj/gateway/` (overridable via config).
- Graceful shutdown path exists (`SIGINT`/`SIGTERM` + `gateway.shutdown`).

**Acceptance:**
- `jj gateway` starts and logs listening address.
- `jj gateway` persists state and can be restarted without losing sessions.
...
**Acceptance:**
- `jj chat` can have a real conversation with streamed output.
- On provider error, the run ends cleanly and an `error` entry is appended.

---

### 17.6 CLI REPL `jj chat` (required)
- `jj chat` opens `main` by default; `--session <key>` switches sessions.
